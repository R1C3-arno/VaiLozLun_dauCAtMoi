...
def compute_TC_total(Q, P, n, k1, Av, theta, params, ts, tT, CR_ts):
    ...
    TV3 = D * CP
    # Investment cost: Chỉ tính nếu thực sự đầu tư (nhỏ hơn giá trị ban đầu)
    TV4 = B1 * math.log(theta / theta0) if theta0 < theta else 0
...
# --- Solver Functions ---
def giai_nghiem_daohamTc_theoP(b4_val, params):
    D = params["D"]
    xi1 = params["xi1"]
...
if __name__ == "__main__":
    ...
    while n <= MAX_N:
        ...
        for j in range(1, m + 1):
            ...
            # 🛑 CỜ TRẠNG THÁI: Biến nào bị True nghĩa là đã bị khóa cứng ở biên
            fix_Av = False
            fix_theta = False
            ...
            while True:
                ...
                for iter in range(50):
                    ...
                    # 1. Tính Betas (Dùng Av, theta hiện tại - dù là fix hay chưa fix)
                    b1_val = b1(params["D"], n, params["A0"], params["CT"], params["pi"], params["sigma"], ts, Q, P,
                                params["tT"], k1, CR, Av)
                    b2_val = b2(params["D"], params["pi"], params["sigma"], ts, Q, P, params["tT"], k1, n)
                    b3_val = b3(params["hb"], params["sigma"], k1, ts, Q, P, params["hv"], n, params["D"],
                                params["rho"], theta)
